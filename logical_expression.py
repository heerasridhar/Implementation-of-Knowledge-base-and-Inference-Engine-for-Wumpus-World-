...
def tt_check_all(kb, alpha, symbols, model):
    ...
    first = symbols[0]
    rest = symbols[1:]
    return tt_check_all( kb, alpha, rest, extend(model, first, True) ) \
           and tt_check_all( kb, alpha, rest, extend(model, first, False) )
...
def pl_true(expression, model):
    if expression.connective[0].lower() == 'and':
        ...
        for i, subexpression in enumerate(expression.subexpressions):
            if(i == 0):
                result = pl_true(subexpression, model)
                continue;
            result = result and pl_true(subexpression, model)
        return result
    elif expression.connective[0].lower() == 'not':
        result = not pl_true(expression.subexpressions[0], model)
        return result
        ...
        for i, subexpression in enumerate(expression.subexpressions):
            if(i == 0):
                result = pl_true(subexpression, model)
                continue;
            result = result or pl_true(subexpression, model)
        return result
        ...
        for i, subexpression in enumerate(expression.subexpressions):
            if(i == 0):
                result = pl_true(subexpression, model)
                continue;
            result = result ^ pl_true(subexpression, model)
        return result
    elif expression.connective[0].lower() == 'if':
        left = expression.subexpressions[0]
        right = expression.subexpressions[1]
        statement1 = pl_true(left, model)
        statement2 = pl_true(right, model)
        ...
        statement1 = pl_true(left, model)
        statement2 = pl_true(right, model)
        return ((not statement1) or statement2) and ((not statement2) or statement1)
    return model[expression.symbol[0]]


def tt_entails(knowledge_base, statement, negation, symbollist):
    ...
    list1 = []
    list2 = []
    model = symbollist.copy();
    read_symbols(knowledge_base, list1)
    read_symbols(statement, list2)
    list1 = list(set(list1))
    list2= list(set(list2))
    list1.extend(list2)
    symbols = list(set(list1))
    for s in model.keys():
        try:
            symbols.remove(s)
        except Exception:
